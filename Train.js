...
const TrainComponent = () => {
  const trainInfo = JsonCardData.Train;
  ...
  return (
    <div className="container mx-auto mt-8 max-w-screen-xl p-5">
      ...
      <div className="w-full  md:w-1/2 lg:w-1/2 mt-4 md:mt-0">
          <img src={trainInfo.right.image_src} alt="Train Image" className="w-full" />
        </div>

        <div className="w-full md:w-1/2 lg:w-1/2 mt-4 md:mt-0">
          <h1 className="text-3xl font-bold mb-4 mx-auto text-dark">
            {trainInfo.left.content_1}
          </h1>
          <div className="flex mb-4">
            <p className="text-md text-gray-700 mt-4 mb-4">{trainInfo.left.content_2}</p>
          </div>
          <div className="flex bg-gray-200">
            {trainInfo.left.cards.map((card, index) => (
